...
/**
 * Baked offline pool for a fixed mode, or null (e.g. artist-discography).
 * Generated by scripts/generate-static-pools.mjs from a non-throttled IP.
 * Lazily imported so the ~1 MB snapshot is only downloaded when the live API
 * fails — it never weighs down a normal page load.
 */
async function staticPoolFor(mode: GameMode): Promise<Song[] | null> {
  try {
    const mod = await import('./static-pools.json');
    const pools = (mod.default ?? mod) as Record<string, Song[]>;
...
export async function fetchSongPool(mode: GameMode = 'global-all', artistQuery?: string): Promise<Song[]> {
  ...
  try {
    ...
    // The live API is throttled or down. Prefer this user's own recently-cached
    // pool, then the baked offline pool, before giving up. The game stays
    // playable even when Apple is blocking the proxy entirely.
    const stale = readCachedPool(cacheKey, Infinity);
    if (stale) return shuffled(stale);

    const baked = await staticPoolFor(mode);
...
export async function fetchSongPoolFromApi(mode: GameMode, artistQuery?: string): Promise<Song[]> {
